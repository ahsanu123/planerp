...
public class ClaimsReporter
{
    ...
    public ClaimsReporter(RequestDelegate requestDelegate) => next = requestDelegate;
    ...
    public async Task Invoke(HttpContext context)
    {
        ...
        {
            Console.WriteLine(
                $"Auth type: {ident.AuthenticationType}," + $" {ident.Claims.Count()} claims"
            );
            foreach (Claim claim in ident.Claims)
            {
                Console.WriteLine(
                    $"Type: {GetName(claim.Type)}, "
